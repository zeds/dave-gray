...
　eslintrcでrules: { ... 'react/prop-types': ['off'],
...
・cssをしっかり覚えるには、styled-componentsの方がいいと思う。
・Tailwindはライブラリがあるので、開発は早くなりそう。
・styled-componentsは、1から全て作成しなければならない。しかし拡張しやすい。
・styled-componentsは、既存のcssから移行しやすい？
...
githubからダウンロードして、yarn install, yarn startで動いた。
original：react-data-website

Github
https://github.com/itSatoriCode/react-data-website
書き方が古いので、最新のReactに合わせて書き直している。

ディレクトリ配置、ファイル名はこの動画に合わせている
https://www.youtube.com/watch?v=02zO0hZmwnw
・index.htmlにgoogle fontを読み込む
・useHistoryはV6でdeprecatedになった。{useNavigate} from 'react-router-dom'を使う

framer-motionで始めるReactアニメーション
基本的にアニメーションを実装する場合には、Framer Motion を使えば事足ります。
